...
assert(
  'alice cannot update doc-2 (bob owns it)',
  permix.check('doc.update', doc2),
  false
);
...
assert(
  'bob cannot update doc-1 (not owner)',
  permix.check('doc.update', doc1),
  false
);
assert(
  'bob cannot read doc-3 (different team)',
  permix.check('doc.read', doc3),
  false
);
...
assert(
  'charlie cannot read doc-1 (no relation)',
  permix.check('doc.read', doc1),
  false
);
assert(
  'charlie can read doc-2 (write >= read)',
  permix.check('doc.read', doc2),
  true
);
assert(
  'charlie can update doc-2 (write share)',
  permix.check('doc.update', doc2),
  true
);
assert(
  'charlie cannot admin doc-2 (write < admin)',
  permix.check('doc.admin', doc2),
  false
);
...
assert(
  'alice can read doc-3 (read share)',
  permix.check('doc.read', doc3),
  true
);
assert(
  'alice cannot update doc-3 (read < write)',
  permix.check('doc.update', doc3),
  false
);
