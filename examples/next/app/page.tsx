...
export default async function Home() {
  const [session, role, posts] = await Promise.all([getSession(), getDemoRole(), getPosts()])
    ...
    <main className='mx-auto flex w-full max-w-3xl flex-1 flex-col gap-8 px-6 py-10'>
      <header className='space-y-3'>
        ...
        <p className='max-w-2xl text-zinc-600 dark:text-zinc-400'>
          ...
          . Rules are set once in the root layout, checked on the server in pages and route
          handlers, then dehydrated for client components.
        </p>
      </header>
      ...
      <section className='rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900'>
        ...
        <div className='mt-4 flex flex-wrap gap-2'>
          <PermissionBadge label='post.create' allowed={permix.check('post.create')} />
          <PermissionBadge
