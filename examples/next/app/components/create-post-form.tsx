...
export function CreatePostForm({ canCreate }: { canCreate: boolean }) {
  ...
  async function handleSubmit() {
    ...
    setResult(response.ok ? data.message : `${response.status}: ${data.error ?? 'Request failed'}`)
  }
  ...
  return (
    <section className='rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900'>
      ...
      <p className='mt-2 text-sm text-zinc-600 dark:text-zinc-400'>
        ...
        on the server. Server says create is <strong>{canCreate ? 'allowed' : 'denied'}</strong> for
        this request.
      </p>
      <button
        type='button'
