...
<Callout>
  In most cases you should use `check` directly. Use `isReadyAsync()` when you need to ensure
  permissions are ready before checking, for example in route middleware.
</Callout>
...
<Callout type='warn'>
  You can still check permissions without providing the data, but dynamic rules that require entity
  data will return `false` in that case.
</Callout>
