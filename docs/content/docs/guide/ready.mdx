...
<Callout type='warn'>
  `hydrate()` alone does **not** make the instance ready. You must call `setup()` after hydration to
  restore function-based rules and mark the instance as ready.
</Callout>
...
<Callout>
  Read more about [hydration](/docs/guide/hydration) to learn how to transfer permissions from the
...
<Callout>
  The `ready` event fires **once** on the first `setup()` (or when initial rules are passed to
  `createPermix`). Register `permix.hook('ready', ...)` before that first `setup()` if you need to
  react to readiness — not after a later `setup()` on an already-ready instance.
</Callout>
