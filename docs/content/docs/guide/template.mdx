...
<Callout>
  Templates are type-checked at compile time, ensuring your permission definitions match the
  instance definition.
</Callout>
...
function userPermissions(userId: string, role: 'admin' | 'user'): Rules<PermissionsDefinition> {
  return {
    post: {
      create: role === 'admin',
      read: true,
