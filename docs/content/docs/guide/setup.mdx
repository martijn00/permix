...
<Callout type='warn'>
  You always should describe all permissions in the `setup` method that was defined in the Permix
  generic type.
</Callout>

<Callout>For role separation, you can use the [`template`](/docs/guide/template) method.</Callout>
...
<Callout>
  ...
  [Enum-based](https://github.com/letstri/permix/tree/main/examples/enum-based) for more
  information.
</Callout>
...
permix.setup({
  comment: {
    update: (comment: { authorId: string }) => comment.authorId === currentUser.id,
  },
})
```
...
<Callout>See the [template](/docs/guide/template) for more examples and patterns.</Callout>
...
<Callout>
  The `getRules` method returns the exact rules object that was set using `setup`, including any
