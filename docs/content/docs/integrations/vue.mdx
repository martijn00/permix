...
<Callout>
  Before getting started with Vue integration, make sure you've completed the initial setup steps in
  the [Quick Start](/docs/quick-start) guide.
</Callout>
...
<Callout>
  Remember to always pass the same Permix instance to both the `PermixProvider` and `usePermix`
...
<template>
  ...
  <Check path="entity.action" :data="data" reverse>
    ...
    <template #otherwise> Will show this if a user doesn't have permission </template>
  </Check>
</template>
```
...
<template>
  <div>
    ...
    <Check path="post.edit" :data="props.post">
      ...
      <template #otherwise> You don't have permission to edit this post </template>
    </Check>
  </div>
</template>
```
