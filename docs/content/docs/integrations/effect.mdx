...
<Callout>
  Before getting started with the Effect integration, make sure you've completed the initial setup
...
<Callout type='warn'>
  `hydrate` does not mark the instance ready and cannot restore function-based rules. Follow with
  `setup` using the full rule set when you need dynamic checks or `isReady`. See the [Hydration
...
Effect.runPromise(program.pipe(Effect.provide(PermixLive), Effect.provide(CurrentUserLive))).then(
  console.log
)
