...
<Callout>
  Before getting started with oRPC integration, make sure you've completed the initial setup steps
...
<Callout>
  Call `.contextKey('name')` on the returned builder to set a custom context key — its literal type
  is inferred automatically. Omit it to use the default key `'permix'`. Pass `onForbidden` as an
  option to `createPermix` itself.
</Callout>
...
const protectedMiddleware = orpcPermix.use(({ context, next }) => {
  return next({
    context: permix.setupContext(context.user.role === 'admin' ? adminTemplate() : userTemplate()),
  })
})
```
