...
<Callout>
  Before getting started with tRPC integration, make sure you've completed the initial setup steps
...
const protectedProcedure = t.procedure.use(({ ctx, next }) => {
  return next({
    ctx: permix.setupContext(ctx.user.role === 'admin' ? adminTemplate() : userTemplate()),
  })
})
```
