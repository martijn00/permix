...
<Callout>
  Before getting started with the server integration, make sure you've completed the initial setup
  steps in the [Quick Start](/docs/quick-start) guide.
</Callout>
...
serve({
  ...
  fetch(req) {
    // Check a single path
    return permix.checkMiddleware('post.create')(req, () => Response.json({ success: true }))
  },
})
```
...
const permix = createPermix<Definition>({
  onForbidden: () => Response.json({ error: 'Custom forbidden message' }, { status: 403 }),
})
```
...
const permix = createPermix<Definition>({
  onForbidden: ({ req, path, data }) => {
    if (path === 'post.create') {
      return Response.json({ error: `You don't have permission for ${path}` }, { status: 403 })
    }
