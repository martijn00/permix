...
<Callout>
  Before getting started with Fastify integration, make sure you've completed the initial setup
...
<Callout>
  The plugin preserves full type safety from your Permix definition, ensuring your permission checks
  are type-safe.
</Callout>
...
fastify.put(
  '/posts/:id',
  {
    preHandler: permix.checkMiddleware((c) => c('post.read') && c('post.update')),
  },
  (request, reply) => {
    reply.send({ success: true })
  }
