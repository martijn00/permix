...
<Callout>
  Before getting started with Svelte integration, make sure you've completed the initial setup steps
  in the [Quick Start](/docs/quick-start) guide. Svelte integration requires Svelte 5.
</Callout>
...
<Callout>
  `usePermix` returns an object with a reactive `isReady` getter and a `check` method. Access them
  directly on the returned object (for example `permissions.isReady`) to keep reactivity — don't
  destructure `isReady`.
</Callout>
