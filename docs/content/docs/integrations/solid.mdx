...
<Callout>
  Before getting started with Solid integration, make sure you've completed the initial setup steps
...
export function App(props: { dehydratedState: DehydratedState<any>; children: any }) {
  onMount(() => {
  ...
  return (
    <PermixProvider permix={permix}>
      <PermixHydrate state={props.dehydratedState}>{props.children}</PermixHydrate>
    </PermixProvider>
  )
}
```
