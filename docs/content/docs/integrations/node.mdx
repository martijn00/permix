...
<Callout>
  Before getting started with Node.js integration, make sure you've completed the initial setup
...
const server = http.createServer(async (req, res) => {
  ...
  if (path === '/posts' && method === 'POST') {
    ...
    await permix.checkMiddleware((c) => c('post.read') && c('post.update'))(req, res, next)
